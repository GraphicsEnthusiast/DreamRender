...
public:
	...
	static SampledSpectrum FromSampled(const float* lambda, const float* v, int n) {
		...
		for (int i = 0; i < nSpectralSamples; ++i) {
			...
			float lambda0 = glm::mix(sampledLambdaStart, sampledLambdaEnd, float(i) / float(nSpectralSamples));
			float lambda1 = glm::mix(sampledLambdaStart, sampledLambdaEnd, float(i + 1) / float(nSpectralSamples));
	...
	static void Init() {
		...
		for (int i = 0; i < nSpectralSamples; ++i) {
			float wl0 = glm::mix(sampledLambdaStart, sampledLambdaEnd, float(i) / float(nSpectralSamples));
			float wl1 = glm::mix(sampledLambdaStart, sampledLambdaEnd, float(i + 1) / float(nSpectralSamples));
