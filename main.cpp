...
int main() {
	...
	medium = NULL;
	float sigma_a2[3] = { 1.0f, 1.0f, 1.0f };
	float sigma_s2[3] = { 0.5f, 0.5f, 0.5f };
	float scale2 = 200.0f;
	...
	Transform tran2 = Transform::Translate(0.0f, 1.0f, -1.0f) * Transform::Scale(0.3f, 0.3f, 0.3f) * Transform::Rotate(0.0f, 90.0f, 0.0f);
	Pinhole camera(Point3f(0.0f, 0.0f, -4.0f), Point3f(0.0f), Vector3f(0.0f, 1.0f, 0.0f), 1.0f, 60.0f, (float)Width / (float)Height, medium);
	//Thinlens camera2(Point3f(10.0f, 8.0f, 10.0f), Point3f(0.0f), Vector3f(0.0f, 1.0f, 0.0f), 1.0f, 60.0f, (float)Width / (float)Height, 2.0f);
	PostProcessing post(std::make_shared<Reinhard>(), 2.0f);
	...
	float radiance[3] = { 10.0f, 10.0f, 10.0f };
...
// 	auto material12 = std::make_shared<Mixture>(material8, material6, 0.5f);
	...
	scene.AddShape(new Quad(material4, Point3f(-10.0f, -0.55f, -10.0f), Vector3f(20.0f, 0.0f, 0.0f), Vector3f(0.0f, 0.0f, 20.0f)));
...
// 	scene.AddLight(std::make_shared<SphereArea>(new Sphere(material2, Point3f(1.0f, 1.0f, -1.0f), 0.25f, medium)));
	scene.AddLight(std::make_shared<TriangleMeshArea>(new TriangleMesh(material2, "teapot.obj", tran2, medium)));
...
//	scene.AddLight(std::make_shared<InfiniteArea>(std::make_shared<Hdr>("spruit_sunrise_4k.hdr"), 1.0f));
	...
	VolumetricPathTracing vpt(std::make_shared<Scene>(scene), std::make_shared<Independent>(), std::make_shared<Gaussian>(), Width, Height, 1024);
