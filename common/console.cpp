...
void Console::ClearLog() {
	std::unique_lock<std::mutex> lock(log_mutex_); // Lock for thread safety
...
void Console::AddLog(const char* fmt, ...) IM_FMTARGS(2) {
	...
	va_list args;
	va_start(args, fmt);
	vsnprintf(buf, IM_ARRAYSIZE(buf) - 1, fmt, args);
...
void Console::Draw() {
	...
	if (ImGui::BeginChild("ScrollingRegion", ImVec2(0, -footer_height_to_reserve), ImGuiChildFlags_None,
		...
		// Make a copy of log items for safe rendering
		ImVector<const char*> items_copy;
		{
			std::unique_lock<std::mutex> lock(log_mutex_); // Lock for thread safety
			items_copy.resize(items_.size());
			for (int i = 0; i < items_.size(); ++i) {
				items_copy[i] = Strdup(items_[i]);
			}
		}

		// Render log items with color coding
		for (const char* item : items_copy) {
			if (!item) {
		...
		if (0 == candidates.Size) {
			...
			for (;;) {
				...
				for (int i = 0; i < candidates.Size && all_candidates_matches; i++) {
					...
					else if (0 == c || std::toupper(candidates[i][match_len]) != c) {
