...
SampledSpectrum DirectCameraVisibility(vec3 position, CameraSampleInfo camera_sample_info, SampledSpectrum beta, 
    ...
    float shadow_distance_remaining = length(camera_sample_info.ray.origin - position) - Epsilon;
    vec3 shadow_origin = position;
    SampledSpectrum visibility = SampledSpectrumNewFloat(1.0f);
    
    while (true) {
        Ray shadow_ray = SpawnShadowRay(shadow_origin, camera_sample_info.ray.direction, shadow_distance_remaining);
        Hit occlusion_hit = BVHTraverse(shadow_ray);
 ...
 * @param pixel_coords Current pixel coordinates (unused in light tracing, kept for compatibility)
 * @param camera Camera parameters
 * @param sobol_sampler Pre-initialized Sobol quasi-random sequence sampler for Monte Carlo integration
 * @param lambda Pre-sampled wavelengths for spectral rendering; contains wavelength values
 * @param max_bounce Maximum number of ray bounces
 * @return SampledSpectrum representing accumulated power
 */
SampledSpectrum LightTracing(ivec2 pixel_coords, Camera camera, inout SobolSampler sobol_sampler, 
                            SampledWavelengths lambda, float max_bounce) {
    ...
    if (light_ray_info.pdf > 0.0f) {
        beta = DivFloat(light_ray_info.emission_cosine, light_ray_info.pdf);
    }
    ...
    for (int bounce = 0; bounce < max_bounce; ++bounce) {
        ...
        if (info.has_medium) {
            ...
            if (medium_sample_info.scattered) {
                ...
                CameraSampleInfo camera_sample_info = CameraSample(camera, info.position);
                if (camera_sample_info.pdf > 0.0f) {
                    ...
                    // Evaluate phase function
                    PhaseEvalInfo phase_eval_info = PhaseEvaluate(info, world_v, camera_sample_info.ray.direction);
                    
                    if (phase_eval_info.pdf > 0.0f) {
                        // Contribution: β * visibility * phase * we_cosine / camera_pdf
                        SampledSpectrum L_medium = MulFloat(
                            Mul(Mul(beta, visibility), phase_eval_info.phase),
                            camera_sample_info.we_cosine / camera_sample_info.pdf
                        );
                        
                        // Accumulate to camera pixel
                        radiance = Add(radiance, L_medium);
                    }
                }
                // ========================= Volume Camera Sampling =========================
        ...
        if (!info.has_medium || !is_medium_scattered) {
            ...
            // Handle regular surface interaction
                ...
                if (camera_sample_info.pdf > 0.0f) {
                    ...
                    // Evaluate BSDF
                    MaterialEvalInfo mat_eval_info = MaterialEvaluate(info, world_v, camera_sample_info.ray.direction, lambda);
                    
                    if (mat_eval_info.pdf > 0.0f) {
                        // Contribution: β * visibility * bsdf_cosine * we_cosine / (bsdf_pdf * camera_pdf)
                        SampledSpectrum L_surface = MulFloat(
                            Mul(Mul(beta, visibility), mat_eval_info.bsdf_cosine),
                            camera_sample_info.we_cosine / (mat_eval_info.pdf * camera_sample_info.pdf)
                        );
                        
                        // Accumulate to camera pixel
                        radiance = Add(radiance, L_surface);
                    }
                }
                // ========================= Surface Camera Sampling =========================
